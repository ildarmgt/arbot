...
export default function initialBots (st) {
  ...
  st.bots.push({
    ...
    offsetPercent: [0.45], // % offset from ref price for bids and asks (if no stdev used)
    positionFraction: [0.50], // fraction of available coins to place order with
    minSTDEV: 0.25, // min standard deviation for use (has to do with fees per trade & sample quality)
    offsetSTDEV: [1], // if stdev is used, what multipliers to use up and down
  ...
  st.bots.push({
    ...
    sourceTrade: 'cryptopia', // trade here
    sourceTradeDelay: 300, // delay between API calls (ms) for trading exchange
    offsetPercent: [0.45], // percent offset from reference price for bids and asks
    positionFraction: [0.50], // fraction of available coins to place order with
    minSTDEV: 0.25, // min standard deviation for use (has to do with fees per trade & sample quality)
    offsetSTDEV: [1], // if stdev is used, what multipliers to use up and down
